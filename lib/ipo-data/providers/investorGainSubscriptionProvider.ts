...
const SOURCE_URL = "https://www.investorgain.com/report/ipo-subscription-live/333/";
...
function parseRows(html: string): SubscriptionDataPoint[] {
  ...
  for (const table of parseTables(html)) {
    ...
    for (const row of table.rows) {
      ...
      if (!ipoName || /no data|issuer company/i.test(ipoName)) {
        continue;
      }
...
export const investorGainSubscriptionProvider: PublicDataProvider<SubscriptionDataPoint> = {
